...
<body class="bg-gray-100">
    ...
    <div class="w-full max-w-md mx-auto mt-10">
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="POST" action="{{route('dashboard.trips.storeagain',$trip->id)}}" enctype="multipart/form-data">
            @csrf
            @if ($errors->any())
            <div class="mb-4">
        ...
        @endif
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" type="text" placeholder="Enter Trip Name" name="{{$key}}[name]" value="{{$trip->translate($key)->name}}">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="description">
                    معلومات الرحلة
                </label>
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" placeholder="Enter Trip Info" name="{{$key}}[info]" id="editor{{$key}}">{!!$trip->translate($key)->info!!}</textarea>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="title">
                    مكان الرحلة
                </label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" type="text" placeholder="Enter Trip Area" name="{{$key}}[area]" value="{{$trip->translate($key)->area}}">
            </div>
            @endforeach
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="image">
